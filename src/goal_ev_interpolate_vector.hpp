#ifndef GOAL_EV_INTERPOLATE_VECTOR_HPP
#define GOAL_EV_INTERPOLATE_VECTOR_HPP

/** \file goal_ev_interpolate_vector.hpp */
...
/** \brief Interpolate a vector nodal field to integration points.
  * \details This evaluator will fill in multidimensional arrays for the
  * values and gradients of a nodal vector field interpolated to the
  * integration points of an element.
  *
  * dependent fields  | data layout
  * ----------------  | -----------
  * shape             | (Elem, Node, IP, Dim)
  * grad_shape        | (Elem, Node, IP, Dim, Dim)
  * nodal             | (Elem, Node, Dim)
  *
  ...
  *
  * field descriptions:
  * - shape, The input vector field's shape function values, as determined
  * by the class \ref goal::VectorShape.
  * - grad_shape, The input vector field's shape function gradient values
  * as determined by the class \ref goal::VectorShape.
  * - nodal, The input field's nodal values gathered by the class
  * goal::GatherVector.
  * - u, The values of the filed interpolated to the integration points
  * which are determined by the method \ref goal::Field::get_q_degree.
...
class InterpolateVector : public PHX::EvaluatorWithBaseImpl<TRAITS>,
 ...
 private:
  ...
  PHX::MDField<const double, Elem, Node, IP, Dim> shape;
  PHX::MDField<const double, Elem, Node, IP, Dim, Dim> grad_shape;
  PHX::MDField<const ScalarT, Elem, Node, Dim> nodal;

  PHX::MDField<ScalarT, Elem, IP, Dim> u;
