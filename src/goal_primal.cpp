...
static void make_soln(Physics* phy, Evaluators& r, Evaluators& j) {
  auto f = phy->get_soln();
...
Primal::Primal(ParameterList const& p, Physics* phy) {
  params = p;
  physics = phy;
  sol_info = 0;
  make_soln(physics, residual, jacobian);
  physics->build_resid<ST>(residual);
  physics->build_resid<FADT>(jacobian);
}
