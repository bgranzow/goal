#include "gmodel.hpp"

int main()
{
  using namespace gmod;
  default_size = 8.0;
  auto p0 = new_point2(Vector{0, 0, 0});
  auto p1 = new_point2(Vector{48, 44, 0});
  auto p2 = new_point2(Vector{48, 60, 0});
  auto p3 = new_point2(Vector{0, 44, 0});
  auto p4 = new_point2(Vector{44, 55, 0});
  auto l0 = new_line2(p0, p1);
  auto l1 = new_line2(p1, p2);
  auto l2 = new_line2(p2, p3);
  auto l3 = new_line2(p3, p0);
  auto loop = new_loop();
  add_use(loop, FORWARD, l0);
  add_use(loop, FORWARD, l1);
  add_use(loop, FORWARD, l2);
  add_use(loop, FORWARD, l3);
  auto f = new_plane2(loop);
  embed(f, p4);
  write_closure_to_geo(f, "cooks.geo");
  write_closure_to_dmg(f, "cooks.dmg");
}
