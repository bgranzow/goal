...
static void make_soln_adj(Disc* d, Evaluators& a) {
  auto m = d->get_apf_mesh();
  auto f = m->findField("u");
  GOAL_DEBUG_ASSERT(f);
  auto u = rcp(new Soln<FADT>(f, ADJOINT));
  auto w = rcp(new ScalarWeight(f));
  a.push_back(u);
  a.push_back(w);
}

static void make_soln_err(Disc* d, Evaluators& e) {
  auto m = d->get_apf_mesh();
...
void NestedAdjoint::build_data() {
  ...
  zu_coarse = apf::createFieldOn(nested_mesh, "u_z_coarse", apf::SCALAR);
  zu_fine = apf::createFieldOn(nested_mesh, "u_z_fine", apf::SCALAR);
