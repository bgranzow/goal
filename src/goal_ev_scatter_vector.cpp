...
ScatterVector<goal::Traits::Residual, TRAITS>::ScatterVector(
  ...
  resid = PHX::MDField<const ScalarT, Elem, Node, Dim>(
...
template <typename TRAITS>
void ScatterVector<goal::Traits::Residual, TRAITS>::postRegistrationSetup(
...
template <typename TRAITS>
void ScatterVector<goal::Traits::Residual, TRAITS>::preEvaluate(PreEvalData i) {
  info = rcpFromRef(i);
  assert(info->ghost->R != Teuchos::null);
}

template <typename TRAITS>
void ScatterVector<goal::Traits::Residual, TRAITS>::evaluateFields(
    EvalData workset) {
  auto R = info->ghost->R;
  ...
  for (int elem = 0; elem < workset.size; ++elem) {
    ...
    for (int node = 0; node < num_nodes; ++node) {
      for (int dim = 0; dim < num_dims; ++dim) {
        LO row = indexer->get_ghost_lid(field_idx, e, node, dim);
        R->sumIntoLocalValue(row, resid(elem, node, dim));
      }
    }
  }
}
...
template <typename TRAITS>
ScatterVector<goal::Traits::Jacobian, TRAITS>::ScatterVector(
...
template <typename TRAITS>
void ScatterVector<goal::Traits::Jacobian, TRAITS>::postRegistrationSetup(
    SetupData d, PHX::FieldManager<TRAITS>& fm) {
  this->utils.setFieldData(resid, fm);
  (void)d;
}
...
template <typename TRAITS>
void ScatterVector<goal::Traits::Jacobian, TRAITS>::preEvaluate(PreEvalData i) {
  info = rcpFromRef(i);
  assert(info->ghost->R != Teuchos::null);
...
template <typename TRAITS>
void ScatterVector<goal::Traits::Jacobian, TRAITS>::scatter_primal(
...
template <typename TRAITS>
void ScatterVector<goal::Traits::Jacobian, TRAITS>::scatter_adjoint(
...
template <typename TRAITS>
void ScatterVector<goal::Traits::Jacobian, TRAITS>::evaluateFields(
    EvalData workset) {
  auto R = info->ghost->R;
